...
def run(index, neural_net, args_model, dataset_sample, test_size=0.2):
    ...
    model_result.save_encoding_dict(neural_net)
    ...
    if success:
        ...
        model_result.confusion_matrix(neural_net, cutoff_threshold=0.65)
        model_result.roc_curve(neural_net)
        model_result.calc_significance(neural_net.significance_dataset(), num_thresholds=200)
