...
class ModelResults():
    ...
    def save_encoding_dict(self, neural_net):
    ...
    def calc_significance(self, dataset, num_thresholds=200, ZA=True):
        ...
        self.bin_centres_sig = bin_centres_sig
        self.sig_vals = sig_vals
        
        x = [0.5, self.max_significance]
        
        for i in x:
            df_selection = dataset[dataset['labels_pred'].between(i, 1)]
            df_sig = df_selection[df_selection['event_labels'] == 1]
            df_back = df_selection[df_selection['event_labels'] == 0]
            sum_xs_weight_sig = df_sig['xs_weight'].sum()
            sum_xs_weight_back = df_back['xs_weight'].sum()
            ...
            if i == 0.5:
                self.standard_sig = sqrt(2*((s+b)*np.log(1+(s/b))-s))
            else:
                self.boosted_sig = sqrt(2*((s+b)*np.log(1+(s/b))-s))
